...
std::vector<DetectionRect> ObjectDetector::detect(const cv::Mat &input) {
    ...
    auto detections = filterBoxesNMS(boxesInfo);
    return detections;
}

cv::Mat ObjectDetector::detectAndDraw(const cv::Mat &input) {
    cv::Mat result = input.clone();
    std::vector<DetectionRect> detections = detect(result);
    for (int i = 0; i < detections.size(); ++i) {
        DetectionRect detection = detections[i];

        cv::Rect box = detection.getRect();
...
std::vector<DetectionRect>
ObjectDetector::filterBoxesNMS(const BoxesInfo &boxesInfo) {
    std::vector<DetectionRect> detections;
    ...
    for (unsigned long i = 0; i < nmsResult.size(); ++i) {
        ...
        detections.push_back(DetectionRect(box, className, classId, confidence));
    }
    return detections;
}
